    ...
    public class DefaultConnection : IConnection
    {
        ...
        private void SendPacketInternal(Packet packet, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                //AsyncCallback myAsycCallback = new AsyncCallback(IASendData);
                //this._socket.BeginSend(packet.Payload,       //异步发送数据
                //                                0,
        ...
        private void IASendData(IAsyncResult pIAsyncResult)
        {
            int intSend;
            try
            {
                intSend = this._socket.EndSend(pIAsyncResult);    //获得发送数据的长度
            }
            catch
            {
                return;
